...
| Pitch PDF and demo video | PDF in repository and Release; YouTube upload reported by owner | “The repository and GitHub Release contain the ten-page pitch PDF. The project owner reports uploading the two-minute demo video to YouTube; its URL is not stored here and its public availability has not been independently checked. Video, audio, narration, and production sources are absent from the reachable `main` and `v1.0.2` histories. Project version is `v1.0.2`.” |
