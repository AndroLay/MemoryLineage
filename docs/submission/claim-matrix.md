...
| Pitch PDF and demo video | PDF in repository; YouTube upload reported by owner | “The repository contains the ten-slide pitch PDF. The project owner reports uploading the two-minute demo video to YouTube; its URL is not stored here and its public availability has not been independently checked. Video, audio, narration script, and production sources are intentionally absent from the current `main` branch. The immutable `v1.0.2` tag retains the earlier snapshot. Project version is `v1.0.2`.” |
