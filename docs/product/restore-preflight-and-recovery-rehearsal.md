...
| Submission media | Video improves judge access to an incident. | The current worktree contains a ten-slide PDF, a 120-second local Chromium recording with an eased cursor, captions, interface cues, and synthesized English narration, plus an earlier 37-second cue-only cut. The narration is synthetic and still needs human listening review. The current PDF/video are uncommitted, not uploaded to Devpost, and not verified against the hosted site. | Do not claim superiority on video/pitch media without judge evidence. Keep the local artifact and hosted site status separate. |
