...
| Submission media | A short walkthrough can make the recovery incident easier to follow. | The ten-page pitch PDF remains in the repository and GitHub Release. The project owner reports that the two-minute demo was uploaded to YouTube; its URL is not recorded here. Video and production files are absent from the reachable `main` and `v1.0.2` histories. | Do not claim judge-score improvement without evidence. Keep owner-reported YouTube upload separate from verified public availability. |
...
The public static site is published separately. This work does not add an agent framework integration or staging environment, deploy a new space/contract, or claim production readiness.
