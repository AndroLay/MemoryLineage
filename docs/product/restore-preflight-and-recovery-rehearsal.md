...
| Submission media | A short walkthrough can make the recovery incident easier to follow. | The pitch PDF remains in the repository. The project owner reports that the two-minute demo was uploaded to YouTube; its URL is not recorded here. Video and production files are absent from current `main`; the immutable `v1.0.2` tag retains the earlier snapshot. | Do not claim judge-score improvement without evidence. Keep owner-reported YouTube upload separate from verified public availability. |
